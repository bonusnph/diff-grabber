...
input MasterSide input_master_side          = SIDE_AUTO;     // Scope: Master — master direction (Slave auto-opposite)
input double input_lot                      = 0.01;          // Scope: Master — lot size for orders (applies to Master and Slave)
double g_max_lots                     = 0.3;           // Maximum lot size per order (edit here)
...
string input_close_th_time2               = "02:20";  // HH:mm — schedule slot 2
int    input_close_th_value2              = 5;       // points — threshold at time2
string input_close_th_time3               = "02:40";  // HH:mm — schedule slot 3
int    input_close_th_value3              = 0;        // points — threshold at time3
string input_close_th_time4               = "03:00";  // HH:mm — schedule (prevent close time)
...
string input_force_close_time             = "02:30";  // Scope: Master — time to force close all (HH:mm)
...
{
   if(g_max_lots <= 0.0 || lots <= g_max_lots)
      return lots;
   return g_max_lots;
}
...
bool PlaceOrder(const bool isBuy, const double lots, ulong &ticket_out, double &price_out)
{
   ticket_out = 0;
   price_out = 0.0;
   const double volume = ClampLots(lots);
