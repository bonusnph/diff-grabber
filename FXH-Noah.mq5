...
int    input_close_cooldown_seconds   = 1200;            // Scope: Master — close cooldown after both sides opened
...
// ========================================
bool   input_tp_active_diff_close_enabled = true; // Scope: Master — L1 Enable TP for active diff close
input int    input_tp_active_diff_close_points = 500;     // Scope: Master — L1 TP points threshold to activate diff close
bool   input_sl_active_diff_close_enabled = true; // Scope: Master — L1 Enable SL for active diff close
input int    input_sl_active_diff_close_points = 500;     // Scope: Master — L1 SL points threshold to activate diff close

// ========================================
// TP/SL Active Diff Close Level-2 (Master only)
// Instant close — no averaging/stability, no zone check
// ========================================
bool   input_tp_active_diff_close_l2_enabled = true; // Scope: Master — L2 Enable TP for instant diff close
input int    input_tp_active_diff_close_l2_points = 1000;    // Scope: Master — L2 TP points threshold for instant diff close
bool   input_sl_active_diff_close_l2_enabled = true;  // Scope: Master — L2 Enable SL for instant diff close
...
{
   return (int)MathRound(MathMax(0.0, g_peer_ask - g_peer_bid) / g_point);
}
   ...
   if(input_time_gate_enabled)
   {
      ...
      if(diffOpen >= enterTh)
      {
         ...
         if(input_time_gate_timeout_ms > 0 && held > (ulong)input_time_gate_timeout_ms)
         {
            ...
            LogEvent("TG_OPEN_TIMEOUT", StringFormat("held=%llu;th=%d", held, GetOpenThresholdPoints()));
            return;
         }
         if(held >= (ulong)input_time_gate_open_ms)
         {
            ...
            LogEvent("TG_OPEN_CONFIRMED", StringFormat("diff=%.1f;held=%llu;th=%d", diffOpen, held, GetOpenThresholdPoints()));
         }
         else
         {
            return;
      ...
      if(input_time_gate_enabled)
      {
         ...
         if(diffClose >= enterTh)
         {
            ...
            if(input_time_gate_timeout_ms > 0 && held > (ulong)input_time_gate_timeout_ms)
            {
               ...
               LogEvent("TG_CLOSE_TIMEOUT", StringFormat("held=%llu;th=%d", held, GetCloseThresholdPoints()));
               return;
            }
            if(held >= (ulong)input_time_gate_close_ms)
            {
               triggerClose     = true;
               triggerSource    = "AUTO_TIME_GATE";
               g_tg_close_above = false;
               g_tg_close_fired = true;
               if(input_zone_stability_enabled) ResetZoneStability(false);
               LogEvent("TG_CLOSE_CONFIRMED", StringFormat("diff=%.1f;held=%llu;th=%d", diffClose, held, GetCloseThresholdPoints()));
            }
            else
            {
               return;
