...
void UpdateScheduledCloseOnlyMode()
{
   ...
   // Update caches for Saturday schedule
  ...
  if(!manualDrop && g_last_peer_open_ack_ms>0 && (NowMs()-g_last_peer_open_ack_ms) < (ulong)input_ack_timeout_ms) { g_prev_self_pairs=selfNow; g_prev_peer_pairs=peerNow; return; }
   ...
   if(diff==1)
   {
      if(self < peer)
      {
         ...
         string cmd_id = NewCmdId(); ulong created_ms = NowMs(); int expire_ms = input_cmd_expire_ms;
         LogEvent("CLOSE_ONE_COMMAND", StringFormat("cmd_id=%s;seq=%I64d;source=RECONCILE_PEER_EXTRA;pair_id=%s", cmd_id, (long)g_seq, pickPair));
...
void SlaveLocalReconcile()
{
   ...
   static ulong g_slave_last_open_ms = 0;
   static string g_slave_last_processed_cmd = "";
   ...
   // Skip reconcile for 5 seconds after slave processes an open command
   if(g_slave_last_open_ms > 0 && (NowMs() - g_slave_last_open_ms) < 5000) return;
...
void MasterOpenNow()
{
   ...
   FileWriteAllAtomic(PathOpenCmd(), line);
   LogEvent("OPEN_CMD", StringFormat("cmd_id=%s;side=%s;lotM=%.2f;lotS=%.2f;expire_ms=%d", cmd_id, ((input_master_side==SIDE_BUY)?"BUY":"SELL"), input_lot_master, input_lot_slave, expire_ms));
   g_waiting_slave_open_ack = true; g_pending_open_cmd_id = cmd_id; g_pending_open_created_ms = created_ms; g_rollback_initiated=false;
