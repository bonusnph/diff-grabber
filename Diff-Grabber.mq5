...
int    input_magic_number_base        = 123456;        // Scope: Master — magic base per channel/symbol
...
double input_min_balance_usd           = 0.00;          // Scope: Master — minimum balance required on BOTH peers to allow new open
...
void MasterReconcilePositions()
{
  ...
  // Deterministic pair-wise diff first: act on any specific missing/excess pair id (one per cycle)
  {
    ...
    if((NowMs() - last_reconcile_action_ms) < 2000) { // 2 second throttle
