...
int       I_ORDER_CLOSE_RETRY_COUNT = 8;             // PositionClose attempts per call (requote/off quotes/etc.)
...
// Diff stream + autosync (MASTER evaluates; SLAVE streams SLAVE;...~)
...
int       G_DIFF_SLAVE_TRADE_MODE = -1; // Set from each SLAVE frame (8 fields); -1 until first accepted frame after init
...
string NormalizeLogTail(const string message_line)
{
   ...
   if(StringFind(message_line, LEGACY) != 0)
      return message_line;
   string tail = StringSubstr(message_line, (int)StringLen(LEGACY));
   StringTrimLeft(tail);
   return tail;
}
...
string SyncLogFileName()
{
   ...
   return StringFormat("%s_%s_Acc%I64d.log", SFX_SYNC_LOG_FILE_STEM, role, AccountInfoInteger(ACCOUNT_LOGIN));
}
...
void SyncClearLogFiles()
{
   if(!I_SYNC_LOG_CLEAR_ON_INIT)
      return;
   const long acct = AccountInfoInteger(ACCOUNT_LOGIN);
   const string fn_master = StringFormat("%s_MASTER_Acc%I64d.log", SFX_SYNC_LOG_FILE_STEM, acct);
   const string fn_slave = StringFormat("%s_SLAVE_Acc%I64d.log", SFX_SYNC_LOG_FILE_STEM, acct);
...
{
   string fn = SyncLogFileName();
   const string dir = "Terminal Data\\MQL5\\Files";
   string hdr = StringFormat(
      "[SFX-SYNC] ========== session START role=%s account=%I64d symbol=%s log=%s (see %s) ==========",
...
string NewTxId()
{
   return StringFormat("%I64d_%I64u_%d", (long)AccountInfoInteger(ACCOUNT_LOGIN), NowMs(), MathRand() % 100000);
}
...
ENUM_ORDER_TYPE SideToOrderType(const ENUM_SIDE side)
{
   return side == SIDE_BUY ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
}
...
bool IsMt5CloseTransientRetcode(const uint rc)
{
   switch(rc)
   {
...
bool IsMt5OpenTransientError(const int err)
{
   const uint rc = (uint)MathMax(0, err);
   switch(rc)
   {
      case TRADE_RETCODE_REQUOTE:
      case TRADE_RETCODE_REJECT:
...
bool CloseTicketIfOpenWithPolicy(const int ticket, const bool retry_transient)
{
   ...
   if((int)PositionGetInteger(POSITION_MAGIC) != OrderMagic())
      return true;
   G_TRADE_CLOSER.SetDeviationInPoints((uint)MathMax(0, I_SLIPPAGE));
...
bool IsTrackedPairPositionLive(const int ticket)
{
   ...
   return (pt == POSITION_TYPE_BUY || pt == POSITION_TYPE_SELL);
}

bool IsTicketOpen(const int ticket)
{
   if(IsTrackedPairPositionLive(ticket))
      return true;
   if(ticket <= 0) return false;
   if(!OrderSelect((ulong)ticket))
      return false;
   if((int)OrderGetInteger(ORDER_MAGIC) != OrderMagic())
      return false;
...
void CloseOnlyUpdateSchedule()
{
   ...
   if(dow > 6)
      dow %= 7;
   int now_m = (int)dt.hour * 60 + (int)dt.min;
...
void DoNotDisturbUpdateSchedule()
{
   ...
   MqlDateTime dnd_dt;
   TimeToStruct(TimeLocal(), dnd_dt);
   int dow = (int)dnd_dt.day_of_week;
   if(dow < 0)
      dow = 0;
   if(dow > 6)
      dow %= 7;
   int now_m = (int)dnd_dt.hour * 60 + (int)dnd_dt.min;
...
void CloseOnlyCloseAllMasterOrders()
{
   ...
   for(int i = (int)PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if((long)PositionGetInteger(POSITION_MAGIC) != magic)
         continue;
      G_TRADE_CLOSER.PositionClose(pt);
   }
}
...
void CloseOnlyTryWeekendCloseAll()
{
   ...
   MqlDateTime z;
   TimeToStruct(TimeLocal(), z);
   if((int)z.day_of_week != 6)
      return;

   const int start_m = CloseOnlyParseHmToMinutes(I_CLOSE_ONLY_WEEKEND_START_SAT);
   if(start_m < 0)
      return;
   const int now_m = (int)z.hour * 60 + (int)z.min;
   if(now_m < start_m)
      return;

   const int sat_key = (int)z.year * 10000 + (int)z.mon * 100 + (int)z.day;
...
void PrintLogMasterCloseBegin(const string reason)
{
   const string reason_out = (reason == "DIFF_CLOSE") ? DiffCloseParenLabel() : reason;
...
void PrintLogMasterCloseIntent(const string tag)
{
   const string reason_disp = (G_CLOSE_REASON == "DIFF_CLOSE") ? DiffCloseParenLabel() : G_CLOSE_REASON;
   string line = StringFormat(
...
void StartOpenTransaction()
{
   ...
   if(I_OPEN_MODE == OPEN_BALANCED)
   {
      ...
      for(int attempt = 1; attempt <= max_attempts; attempt++)
      {
         ...
         if(G_OPEN_MASTER_OK)
            break;
         if(!IsMt5OpenTransientError(err))
         {
...
void StartCloseTransaction(const string reason)
{
   ...
   // Always attempt close by tracked ticket. Do not gate on IsTrackedPairPositionLive(): a false
   // negative would skip CloseTicketIfOpen while leaving master_closed=true, orphaning the master leg.
...
void HandleMasterIncomingPacket(const string msg)
{
   ...
   {
      const string pair_key_in = p[1];
      const int slave_ticket_in = (int)StringToInteger(p[2]);
      const bool slave_open_in = ((int)StringToInteger(p[3]) == 1);
      ...
      {
         const bool clear_state = (!slave_open_in && slave_ticket_in <= 0);
         const bool pair_old_enough =
            (G_PAIR_OPENED_MS > 0 &&
             (NowMs() - G_PAIR_OPENED_MS) >= (ulong)MathMax(0, I_PAIR_CLEAR_CROSSKEY_MIN_AGE_SEC) * 1000UL);
         const bool same_tracked_ticket = (slave_ticket_in > 0 && slave_ticket_in == G_PAIR_SLAVE_TICKET);
...
void HandleSlaveIncomingPacket(const string msg)
{
   ...
   {
      const string txid = p[1];
      const string why = (ArraySize(p) >= 3) ? p[2] : "FORCE_FLAT";
      const int before_n = CountEaOpenOrdersOnSlaveSymbol();
      SlaveCloseAllOrders();
      SlaveWeekendReset();
   ...
   if(p[0] == "OPEN_INTENT" && ArraySize(p) >= 8)
   {
      ...
      ENUM_ORDER_TYPE type = (side == "BUY") ? ORDER_TYPE_SELL : ORDER_TYPE_BUY;
      string ln = StringFormat(
         "[SFX-SYNC] OPEN_INTENT_IN tx_id=%s symbol=%s master_side=%s lot_slave=%.2f exec_type=%d",
         txid, G_SYMBOL, side, lot_slave, (int)type
      );
      ExpertPrintLn(ln);
      SyncLog(ln);
   ...
   if(p[0] == "CLOSE_INTENT" && ArraySize(p) >= 4)
   {
      ...
      string pair_key = p[3];
      string ln = StringFormat(
         "[SFX-SYNC] CLOSE_INTENT_IN tx_id=%s reason=%s pair_key=%s sticket=%d",
...
void SlaveLoop()
{
   ...
   if(G_PEER != NULL && G_PEER.IsSocketConnected())
   {
      ...
      bool slave_pair_open = IsTrackedPairPositionLive(G_SLAVE_PAIR_TICKET);
      const int ea_open_n = CountEaOpenOrdersOnSlaveSymbol();
...
{
   return (TerminalInfoInteger(TERMINAL_DLLS_ALLOWED) != 0 &&
           MQLInfoInteger(MQL_DLLS_ALLOWED) != 0);
}

datetime G_LITE_EXPIRE_LAST_CHECK = 0;
bool     G_LITE_EXPIRED_STOP = false;

datetime LiteServerTime()
{
...
bool LiteExpiryReached()
{
   datetime now = LiteServerTime();
   if(now <= 0)
      return false;
   return (now >= (G_LITE_EXPIRE_DATE + 86400));
}

void LiteAlertExpired()
{
   const string msg = StringFormat(
      "[SFX-SYNC-LITE] Expired after %s (server time). EA stopped.",
      TimeToString(G_LITE_EXPIRE_DATE, TIME_DATE));
   Print(msg);
   Alert(msg);
}

void LiteAlertLotCap()
{
   const string msg = StringFormat(
      "[SFX-SYNC-LITE] I_LOT %.2f exceeds max %.2f. EA stopped.",
      I_LOT, G_LITE_MAX_LOT);
   Print(msg);
   Alert(msg);
}

bool LiteExpireRecheckDue()
{
   datetime now = LiteServerTime();
   if(now <= 0)
      return false;
   if(G_LITE_EXPIRE_LAST_CHECK <= 0)
      return true;
   return ((now - G_LITE_EXPIRE_LAST_CHECK) >= 12 * 60 * 60);
}

int OnInit()
{
   if(LiteExpiryReached())
   {
      LiteAlertExpired();
      return INIT_FAILED;
   }
   if(I_LOT > G_LITE_MAX_LOT)
   {
      LiteAlertLotCap();
      return INIT_FAILED;
   }
   G_LITE_EXPIRE_LAST_CHECK = LiteServerTime();
...
void OnTimer()
{
   if(G_LITE_EXPIRED_STOP)
      return;
   if(LiteExpireRecheckDue())
   {
      G_LITE_EXPIRE_LAST_CHECK = LiteServerTime();
      if(LiteExpiryReached())
      {
         G_LITE_EXPIRED_STOP = true;
         LiteAlertExpired();
         ExpertRemove();
         return;
      }
