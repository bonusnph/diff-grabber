...
#ifndef __FXH_SYNC_HEADER_MQL4_MQH__
#define __FXH_SYNC_HEADER_MQL4_MQH__
...
double DiffPoint()
{
   double pt = MarketInfo(G_SYMBOL, MODE_POINT);
   if(pt <= 0.0)
      pt = Point;
   return pt;
}
...
{
   const int d = (int)MarketInfo(G_SYMBOL, MODE_DIGITS);
   if(d >= 4 && d <= 5)
      return 5;
   if(d >= 2 && d <= 3)
      return 2;
...
{
   RefreshRates();
   G_DIFF_SELF_BID = DiffNormQuotePrice(MarketInfo(G_SYMBOL, MODE_BID));
   G_DIFF_SELF_ASK = DiffNormQuotePrice(MarketInfo(G_SYMBOL, MODE_ASK));
   const datetime tq = (datetime)MarketInfo(G_SYMBOL, MODE_TIME);
   const ulong qm = (ulong)tq * (ulong)1000;
...
string DiffHudQuoteMonitorBlock()
{
   ...
   {
      ResetLastError();
      master_tm = IsTradeAllowed() ? 4 : 0;
   }
   string s = "\n";
   s += StringFormat("QUOTE MONITOR: ticks M=%I64u S=%I64u  ~evt/min M=%.0f S=%.0f\n",
                     G_QMON_MASTER_TICKS, G_QMON_SLAVE_FRAMES, rm, rs);
   s = s + StringFormat("  EMA gap ms M=%.1f S=%.1f  fresh=%dms [%s]  skew=%s  ", G_QMON_EMA_GAP_M, G_QMON_EMA_GAP_S, eff, mode, skewStr) + DiffHudFormatQuoteTugBar(share) + "\n";
   s += StringFormat(
...
bool DiffHudOpenCooldownRemainMs(ulong &remMsOut)
{
   ...
   if(G_DIFF_LAST_OPEN_SIGNAL_MS == 0)
      return false;
   const ulong nw = NowMs();
   const ulong cdMs = (ulong)I_DIFF_OPEN_COOLDOWN_SEC * (ulong)1000;
...
bool DiffHudCloseCooldownRemainMs(ulong &remMsOut)
{
   ...
   if(G_DIFF_LAST_CLOSE_SIGNAL_MS == 0)
      return false;
   const ulong nw = NowMs();
   const ulong cdMs = (ulong)I_DIFF_CLOSE_COOLDOWN_SEC * (ulong)1000;
...
string DiffHudStaleSpreadTags()
{
   ...
   if(om || op)
   {
      if(StringLen(parts) > 0)
         parts = parts + "; ";
      if(om && op)
         parts = parts + StringFormat("spread master %d > max %d, slave %d > max %d",
                                        sSelf, I_DIFF_MAX_SPREAD_SELF, sPeer, I_DIFF_MAX_SPREAD_PEER);
      else if(om)
         parts = parts + StringFormat("spread master %d > max %d", sSelf, I_DIFF_MAX_SPREAD_SELF);
      else
         parts = parts + StringFormat("spread slave %d > max %d", sPeer, I_DIFF_MAX_SPREAD_PEER);
   }
   return parts;
}
...
string DiffHudMasterCommentTail()
{
   ...
   if(DiffHudOpenCooldownRemainMs(remO))
   {
      if(StringLen(out) > 0)
         out += "\n";
      const ulong perO = (ulong)I_DIFF_OPEN_COOLDOWN_SEC * (ulong)1000;
   ...
   if(DiffHudCloseCooldownRemainMs(remC))
   {
      if(StringLen(out) > 0)
         out += "\n";
      const ulong perC = (ulong)I_DIFF_CLOSE_COOLDOWN_SEC * (ulong)1000;
...
string DiffHudSignalOpenProgressBlock()
{
   ...
   else
   {
      ...
      {
         ulong nw = NowMs();
         ulong el = (nw > G_DIFF_RAW_OPEN_START) ? (nw - G_DIFF_RAW_OPEN_START) : 0;
...
string DiffHudSignalCloseProgressBlock()
{
   ...
   else
   {
      ...
      {
         ulong nw = NowMs();
         ulong el = (nw > G_DIFF_RAW_CLOSE_START) ? (nw - G_DIFF_RAW_CLOSE_START) : 0;
         lines += StringFormat(" streak age: %u / %d ms\n\n", (uint)el, I_DIFF_RAW_STABILITY_TIMEOUT_MS);
      }
...
void SlaveSendQuoteStream()
{
   ...
   if(G_SLAVE_DUP_CHANNEL_SHUTDOWN)
      return;
   RefreshRates();
   const double bid = DiffNormQuotePrice(MarketInfo(G_SYMBOL, MODE_BID));
   const double ask = DiffNormQuotePrice(MarketInfo(G_SYMBOL, MODE_ASK));
   const datetime tt = (datetime)MarketInfo(G_SYMBOL, MODE_TIME);
   const ulong qmsc = (ulong)tt * (ulong)1000;
   ...
   {
      ResetLastError();
      slave_tm = IsTradeAllowed() ? 4 : 0;
   }
   SendMsg(G_PEER, StringFormat("SLAVE;%.10f;%.10f;%s;%.5f;%I64u;%d;%I64u", bid, ask, G_SYMBOL, pl, qmsc, slave_tm, NowMs()));
}
