...
bool PlaceOrderMaster(const bool isBuy, const double lots, int &ticket_out, double &price_out)
{
   ...
   for(int r=0; r<retries; ++r)
   {
      ...
      {
         price_out = Ask;
         ticket_out = OrderSend(g_symbol, OP_BUY, volume, price_out, slippage, 0, 0, cmt, g_magic, 0, clrGreen);
      }
      else
      {
         price_out = Bid;
         ticket_out = OrderSend(g_symbol, OP_SELL, volume, price_out, slippage, 0, 0, cmt, g_magic, 0, clrRed);
      }
      if(ticket_out>0)
         return true;
...
void MaybeClosePair()
{
   ...
   if (!l2_blocked && diffClose >= GetCloseThresholdPoints())
   {
      ...
      // Reset L1 confirmation state since L2 is closing
      g_close_pending = false;
      g_close_ok_count = 0;
      g_raw_close_pending = false;
      g_raw_close_stable_count = 0;
   ...
   {
      // === ZONE STABILITY CHECK (applies to all L1 modes) ===
      if(input_zone_stability_enabled)
      {
