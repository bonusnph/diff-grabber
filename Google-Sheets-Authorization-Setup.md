...
- **คอลัมน์ C (max_lots)**: ขนาด lot สูงสุดที่อนุญาต (ไม่บังคับ - ใช้สำหรับ ROLE_MASTER เท่านั้น)
...
**หมายเหตุ:**
- ถ้าไม่กำหนดค่า `max_lots` (เว้นว่างหรือไม่มีคอลัมน์) จะไม่มีข้อจำกัดด้านขนาด lot
- `max_lots` จะตรวจสอบเฉพาะกับ EA ที่ตั้งเป็น ROLE_MASTER เท่านั้น
- ถ้า `input_lot` เกินกว่า `max_lots` ที่กำหนด EA จะแสดง error และไม่สามารถเริ่มทำงานได้
