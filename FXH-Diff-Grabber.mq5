...
input bool   input_time_gate_enabled            = true;        // Scope: Master — enable Time-Gate: diff must hold above threshold for N ms before fire
input int    input_time_gate_open_ms            = 200;          // Scope: Master — ms diff must persist above open threshold before fire
input int    input_time_gate_close_ms           = 200;          // Scope: Master — ms diff must persist above close threshold before fire
input int    input_time_gate_hysteresis_offset  = 5;            // Scope: Master — pts below threshold that resets the timer
input int    input_time_gate_timeout_ms         = 2000;         // Scope: Master — max ms to wait; resets and restarts if exceeded

// Drift Persistence Monitor (DPM — Master only)
input bool   input_dpm_enabled      = true;  // Scope: Master — enable DPM background observer (independent of signal mode)
input bool   input_dpm_csv_enabled  = true;  // Scope: Master — write DPM events to CSV file
input int    input_dpm_hud_rows     = 5;      // Scope: Master — recent events shown on HUD (0 = stats only)
input bool   input_dpm_track_open   = true;   // Scope: Master — observe open-side persistence
input bool   input_dpm_track_close  = true;   // Scope: Master — observe close-side persistence
input int    input_dpm_open_th1     = 15;     // Scope: Master — open observe threshold level 1 (pts)
input int    input_dpm_open_th2     = 25;     // Scope: Master — open observe threshold level 2 (pts)
input int    input_dpm_open_th3     = 40;     // Scope: Master — open observe threshold level 3 (pts)
input int    input_dpm_close_th1    = 15;     // Scope: Master — close observe threshold level 1 (pts)
input int    input_dpm_close_th2    = 25;     // Scope: Master — close observe threshold level 2 (pts)
input int    input_dpm_close_th3    = 40;     // Scope: Master — close observe threshold level 3 (pts)

// Zone Stability Filter (works with all modes - Master only)
input  bool   input_zone_stability_enabled    = false;     // Scope: Master — enable zone stability check for all modes
...
bool ParseAuthorizationData(const string csv_data, const long account_number, 
                            ...
                            int &open_cooldown_out, int &close_cooldown_out, string &min_version_out)
{
   ...
   for(int i = start_line; i < line_count; i++)
   {
      ...
      if(field_count >= 2)
      {
         ...
         long csv_account = StringToInteger(csv_account_str);
         if(csv_account == account_number && IntegerToString(csv_account) == csv_account_str)
         {
            ...
            if(field_count >= 4)
            {
               ...
               {
                  open_cooldown_out = (int)StringToInteger(cooldown_str);
               }
            }
            ...
            if(field_count >= 5)
            {
               ...
               if(StringLen(cooldown_str) > 0)
               {
                  close_cooldown_out = (int)StringToInteger(cooldown_str);
               }
            }
   ...
   g_auth_error_message = StringFormat("Account %I64d not found in authorization list", account_number);
   return false;
}
...
// Perform authorization check via API (MQL5) (includes per-account config)
bool CheckAccountAuthorization()
{
   ...
   if (!ParseAuthorizationData(response, account_num, expires, max_lots, open_cooldown, close_cooldown, min_version))
   {
   ...
   {
      g_api_auth_error = StringFormat("Account %I64d expired on %s", account_num, TimeToString(expires));
      g_api_auth_valid = false;
      g_account_authorized = false;
      g_api_auth_hard_failure = true;
      g_api_auth_consecutive_failures = 0;
...
ulong GetFileAgeFromContent(const string file_path)
{
   ...
   ulong file_timestamp = (ulong)StringToDouble(timestamp_str);
...
void MaybeForceCloseByTime()
{
   ...
   {
      // Generate id up-front; emit cmd only after local close success (except DRY fake-ack)
      string cmd_id = NewCmdId();
      ulong created_ms = NowMs();
      int expire_ms = (DryEnabled() && DryOverrideExpireMs()>0) ? DryOverrideExpireMs() : input_cmd_expire_ms;
      if(expire_ms <= 0) expire_ms = 60000;
      ...
      else
      {
         ...
         {
            LogEvent("CLOSE_LOCAL_FAIL", StringFormat("reason=FORCE_CLOSE_BY_TIME;err=%d", (int)GetLastError()));
         }
      }
       fired_today = true;
   }
...
void MasterReconcilePositions()
{
  ...
  if(g_master_last_open_ms > 0 && grace_elapsed < grace_limit) 
  {
     ...
     if(peerDropTmp2)
     {
        ...
        else{LogEvent("CLOSE_LOCAL_FAIL", StringFormat("reason=RECONCILE;err=%d", (int)GetLastError()));}
        g_prev_self_pairs=selfNowTmp2; g_prev_peer_pairs=peerNowTmp2; 
        return;
     }
     if(selfDropTmp2)
  ...
  if(manualDrop) 
  {
     ...
     LogEvent("RECONCILE_DEBUG", StringFormat("PositionsTotal=%d;peer_file_ok=%s;peer_content_len=%d;peer_alive=%s", 
              PositionsTotal(), peerReadOk?"true":"false", StringLen(TrimAll(peerContent)), g_peer_alive?"true":"false"));
  }
...
void SlaveLocalReconcile()
{
  ...
  // Dynamic grace: compute guard but allow immediate reconcile when self>peer
...
bool ReadPeerQuotes()
{
   ...
   g_peer_quote_ms = (ulong)StringToDouble(StringSubstr(s, 0, p1));
...
bool ReadPeerBalanceFresh(double &bal_out, ulong &ts_out)
{
  ...
  bal_out = StringToDouble(f[1]); ts_out = (ulong)StringToDouble(f[3]);
...
int PeerSpreadOk()
{
   ...
   int sp_points = (int)MathRound(MathMax(0.0, sp) / g_point);
...
{
   return StringFormat("DPM_%d_%s.csv", (int)AccountInfoInteger(ACCOUNT_LOGIN), g_symbol);
}
...
{
   return (g_point > 0.0) ? (int)MathRound(MathMax(0.0, (g_peer_ask - g_peer_bid)) / g_point) : 0;
}
...
void MaybeOpenPair()
{
   ...
   if(IsAuthAPIEnabled() && g_account_expires_at > 0 && TimeCurrent() > g_account_expires_at)
   {
      ...
      g_api_auth_valid = false;
      long account_num = AccountInfoInteger(ACCOUNT_LOGIN);
      g_api_auth_error = StringFormat("Account %d expired on %s", account_num, TimeToString(g_account_expires_at));
      Print("[API-AUTH] ", g_api_auth_error);
      return;
   }
   // Check lot limit
   ...
   {
      long account_num = AccountInfoInteger(ACCOUNT_LOGIN);
      string error_msg = StringFormat("Account %d lot size violation: input_lot=%.2f exceeds max_lots=%.2f", account_num, input_lot, g_account_max_lots);
      Print("[API-AUTH] ", error_msg);
      LogEvent("AUTH_LOT_LIMIT_VIOLATION", StringFormat("account=%d;input_lot=%.2f;max_lots=%.2f", account_num, input_lot, g_account_max_lots));
      return;
   }
...
void MaybeClosePair()
{
   ...
   {
      LogEvent("CLOSE_LOCAL_FAIL", StringFormat("reason=AUTO;err=%d", (int)GetLastError()));
   }
}
...
void SlaveProcessOpenCmd()
{
   ...
   ulong created_ms = (ulong)StringToDouble(fields[9]); int expire_ms = (int)StringToInteger(fields[10]);
...
void WritePositions()
{
  ...
  if(g_prev_position_count >= 0 && current_count < g_prev_position_count)
  {
     ...
     if(g_last_position_change_ms == 0)
     {
        ...
        g_prev_position_count = current_count; // Update but don't log yet
...
void DisplayInit()
{
   ...
   if(input_debug_buttons_enabled && (input_role==ROLE_MASTER))
   {
      ...
      ObjectSetInteger(0, dopen_value, OBJPROP_XDISTANCE, CLOSE_ONLY_BTN_X + 70);
      ...
      ObjectSetInteger(0, dclose_value, OBJPROP_XDISTANCE, CLOSE_ONLY_BTN_X + 70);
...
void UpdateDiffDisplay()
{
   ...
   if(input_master_side == SIDE_AUTO && !g_auto_side_locked)
   {
      ...
      {
         ObjectSetString(0, dopen_value, OBJPROP_TEXT, StringFormat("B:%.1f S:%.1f", dOpenBuy, dOpenSell));
         ObjectSetInteger(0, dopen_value, OBJPROP_COLOR, (dOpenBuy>=dOpenSell?cOpenBuy:cOpenSell));
      }
      if(ObjectFind(0, dclose_value) != -1)
      {
         ObjectSetString(0, dclose_value, OBJPROP_TEXT, StringFormat("B:%.1f S:%.1f", dCloseBuy, dCloseSell));
         ObjectSetInteger(0, dclose_value, OBJPROP_COLOR, clrGray);
      }
      return;
   }
...
void DisplayUpdate()
{
   ...
   if(input_role==ROLE_MASTER)
   {
      ...
      int closeLeft = -1; if(g_last_pair_both_open_time>0){ int el=(int)(TimeCurrent()-g_last_pair_both_open_time); int rem=GetEffectiveCloseCooldownSeconds()-el; if(rem<0) rem=0; closeLeft=rem; }
   ...
   if(input_role==ROLE_MASTER)
   {
         ...
         else
      {
         ...
         if(autoSearching)
            DisplaySetLine(line++, StringFormat("Open B:%.1f S:%.1f  Close B:%.1f S:%.1f  Fresh: %s", dOpenBuy, dOpenSell, dCloseBuy, dCloseSell, freshStatus));
         else
...
void MasterCloseNow()
{
   ...
   {
      LogEvent("CLOSE_LOCAL_FAIL", StringFormat("reason=MANUAL;err=%d", (int)GetLastError()));
   }
   g_last_pair_both_open_time = 0; // Reset cooldown anchor for next cycle
}
...
int OnInit()
{
   ...
   if (IsAPIEnabled())
   {
      ...
      if (IsAuthAPIEnabled())
      {
         ...
         {
            long account_num = AccountInfoInteger(ACCOUNT_LOGIN);
            string error_msg = StringFormat("Account %I64d lot size violation: input_lot=%.2f exceeds max_lots=%.2f", account_num, input_lot, g_account_max_lots);
            Print("[API-AUTH] ", error_msg);
            Alert("EA Lot Size Violation: " + error_msg);
            LogEvent("AUTH_LOT_LIMIT_VIOLATION", StringFormat("account=%I64d;input_lot=%.2f;max_lots=%.2f", account_num, input_lot, g_account_max_lots));
            return(INIT_FAILED);
         }
      }
   ...
   if (IsAPIEnabled())
   {
      ...
      if (IsAuthAPIEnabled())
      {
         ...
         if (!auth_ok)
         {
            ...
            {
               long account_num = AccountInfoInteger(ACCOUNT_LOGIN);
               string error_msg = StringFormat("Account %I64d authorization failed: %s", account_num, g_api_auth_error);
               Print("[API-AUTH] ", error_msg);
               Alert("EA Authorization Failed: " + error_msg);
               LogEvent("AUTH_REVOKED", StringFormat("account=%I64d;error=%s", account_num, g_api_auth_error));
               ExpertRemove();
               return;
            }
         }
         ...
         {
            long account_num = AccountInfoInteger(ACCOUNT_LOGIN);
            string error_msg = StringFormat("Account %I64d expired on %s", account_num, TimeToString(g_account_expires_at));
            Print("[API-AUTH] ", error_msg);
            Alert("EA Authorization Expired: " + error_msg);
            LogEvent("AUTH_EXPIRED", StringFormat("account=%I64d;expires=%s", account_num, TimeToString(g_account_expires_at)));
            ExpertRemove();
            return;
         }
