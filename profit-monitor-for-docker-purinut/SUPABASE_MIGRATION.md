...
เพิ่มค่าเดียวกันในไฟล์ `.env` ที่ server (ไฟล์เดียวกับที่ `docker-compose.yml` อ้างผ่าน `env_file`):
```bash
SUPABASE_URL=https://your-project-id.supabase.co
SUPABASE_ANON_KEY=your-anon-key-here
```
...
### 5. **Deploy**
```bash
yarn docker:release   # build + push image ขึ้น Docker Hub
# แล้ว docker compose pull && docker compose up -d บน server (ดูรายละเอียดใน DEPLOY.md)
```
